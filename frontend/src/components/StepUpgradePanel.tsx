  ...
  return (
    <div className="py-4 space-y-4">
      <div className="text-center space-y-1">
        <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">Upgrade to unlock this lesson</p>
        <p className="text-xs text-slate-500 dark:text-slate-400">
          You've reached your plan's AI budget. Choose a plan to keep learning.
        </p>
      </div>
      ...
      <div className={clsx('grid gap-3', upgradePlans.length === 1 ? 'grid-cols-1' : 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3')}>
        {upgradePlans.slice(0, 3).map(plan => {
          ...
          const isHighlight = plan.plan_id === HIGHLIGHT
