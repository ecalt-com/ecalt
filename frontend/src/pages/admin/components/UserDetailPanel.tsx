            ...
            {(() => {
              ...
              return detail.history.map(r => (
                <div key={r.period_start} className="flex items-center gap-2">
                  <span className="text-xs text-slate-400 w-14 shrink-0">{fmtMonth(r.period_start)}</span>
                  ...
                  <span className="text-xs tabular-nums text-slate-500 w-10 text-right">{fmtCents(r.estimated_cost_cents)}</span>
                  <span className="text-xs tabular-nums text-slate-400 w-10 text-right hidden sm:block">{r.message_count} req</span>
                </div>
              ))
            })()}
          </div>
          ...
          <div className="space-y-1.5">
            {detail.coupons.map(c => (
              <div key={c.code} className="flex items-center gap-3 text-xs bg-slate-50 dark:bg-slate-800/40 rounded-lg px-3 py-2">
                ...
                {!c.is_active && (
                  <span className="text-slate-400 text-xs shrink-0">expired</span>
                )}
              </div>
            ))}
          </div>
